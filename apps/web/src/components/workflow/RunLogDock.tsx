  ...
  if (dock.collapsed) {
    return (
      ...
      >
        ...
        {runningCount > 0 && (
          <span className="text-[11px] font-medium text-primary" title={`${runningCount} running`}>
            {runningCount}
          </span>
        )}
      </aside>
      ...
      <div className="flex-1 overflow-y-auto">
        {runs.length === 0 ? (
          <div className="px-4 py-8 text-[13px] text-muted leading-[1.47]">
            Nothing running.
            <p className="mt-3">
              Agent work appears here with live progress. Leave anytime — runs finish on the server.
            </p>
          </div>
        ) : (
          runs.map((run) => {
            ...
            return (
              <div
                key={run.id}
                className="grid grid-cols-[8px_1fr] gap-2.5 px-4 py-3 border-b border-border/50"
              >
                <span
                  className={cn('mt-1.5 h-2 w-2 rounded-full', STATUS_DOT[run.status] ?? 'bg-border')}
                  aria-hidden
                />
